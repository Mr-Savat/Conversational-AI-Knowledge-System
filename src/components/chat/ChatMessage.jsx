...
const ChatMessage = ({ role, text, time, isLoading = false }) => {
  ...
  const showLoading = isLoading || (isAi && !text);
  ...
  return (
    <div className={`flex w-full mb-5 items-end gap-2.5 ${isAi ? 'flex-row' : 'flex-row-reverse'}`}>
      ...
      {/* Avatar */}
      <div className={`w-7 h-7 min-w-7 rounded-full flex items-center justify-center shrink-0 mb-0.5 ${isAi
      ...
      <div className={`flex flex-col gap-1 max-w-[72%] ${isAi ? 'items-start' : 'items-end'}`}>
        ...
        <div className={`relative px-4 py-2.5 text-[15px] leading-[1.45] tracking-[-0.01em] shadow-sm ${isAi
            ...
            <div className="prose prose-sm dark:prose-invert max-w-none">
              <ReactMarkdown
                ...
                components={{
                  ...
                  // Style code blocks
                  code: ({ children }) => <code className="bg-black/5 dark:bg-white/8 px-1 py-0.5 rounded text-[12px] font-mono">{children}</code>,
                  // Style horizontal rules
                  hr: () => <hr className="my-2 border-black/8 dark:border-white/8" />,
                }}
              >
                {text}
              </ReactMarkdown>
